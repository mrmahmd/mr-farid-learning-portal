create table if not exists public.student_access (
  ...
  access_mode text not null default 'sample' check (access_mode in ('sample', 'grade', 'custom', 'all', 'none')),
...
add column if not exists access_mode text not null default 'sample';
...
check (access_mode in ('sample', 'grade', 'custom', 'all', 'none'));
