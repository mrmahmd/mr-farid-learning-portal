(() => {
  ...
  if (!student.id) student.id = sessionStorage.getItem('p3_student_id') || 'guest';
  if (!student.name) student.name = sessionStorage.getItem('p3_student_name') || 'Student';
  if (!student.className) student.className = sessionStorage.getItem('p3_student_class') || '';

  const key = () => `english-primary3:${student.id}:state:v1`;
  ...
  const save = () => {
    ...
    window.dispatchEvent(new CustomEvent('p3:state', { detail: structuredClone(state) }));
    try {
      window.parent?.postMessage({ type: 'PRIMARY3_PROGRESS', studentId: student.id, state }, '*');
    } catch {
  ...
  const setStudent = (next = {}) => {
    ...
    sessionStorage.setItem('p3_student_id', student.id);
    sessionStorage.setItem('p3_student_name', student.name);
    sessionStorage.setItem('p3_student_class', student.className);
    try {
      state = mergeState(JSON.parse(localStorage.getItem(key()) || 'null'));
    } catch {
      state = mergeState(null);
    ...
    window.dispatchEvent(new CustomEvent('p3:student', { detail: student }));
  };
  ...
  window.Primary3App = window.Primary3App || {};
  window.Primary3App.setStudent = setStudent;
  window.Primary3App.getProgress = () => structuredClone(state);

  window.MrFaridCourseProgress?.connect({
    courseId: 'english-primary-3-first-term',
    getState: () => state,
    setState: (next) => {
    ...
    mergeState,
    onStatus: (info) => window.dispatchEvent(new CustomEvent('p3:cloud-status', { detail: info }))
  });
})();
