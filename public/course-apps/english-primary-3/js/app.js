(() => {
  ...
  const Store = window.Primary3Store;
  ...
  function lessonStations(lesson) {
    return STATIONS.filter((s) => s.id !== 'phonics' || lesson.phonics);
  }
    ...
    app.innerHTML = `
      <div class="app-shell ${drawerOpen ? 'drawer-open' : ''}">
        <aside class="sidebar">
          ...
          <a class="brand" href="#home" aria-label="English Primary 3 Home">
            <div class="brand-icon">📖</div>
            <div><strong>English Primary 3</strong><span>Term 1</span></div>
          </a>
      ...
      <section class="hero-section">
        ...
        <div class="hero-copy">
          ...
          <div class="hero-mini-stats">
            <span>📘 23 Lessons</span><span>🏆 6 Unit Challenges</span><span>⭐ ${overallProgress()}% Complete</span>
          </div>
        </div>
      </section>
        ...
        <div class="review-grid">
          ${C.reviews.map((review, index) => {
            ...
            return `<article class="review-card ${locked ? 'locked' : ''}">
              ...
              <div><span>${review.subtitle}</span><h3>${review.title}</h3><p>60 mixed questions covering vocabulary, grammar, reading, phonics and sentence order.</p></div>
    ...
    page.innerHTML = `
      ...
      <section class="section compact">
        ...
        <article class="unit-challenge-card ${isUnitChallengeUnlocked(unit) ? '' : 'locked'}">
          <div class="challenge-medal">🏆</div><div><span>Final assessment</span><h3>Unit ${unit.id} Challenge</h3><p>50 varied questions to measure vocabulary, grammar, reading and phonics.</p></div>
  ...
  function renderLesson(page, lessonId, stationId) {
    ...
    page.innerHTML = `
      <section class="lesson-banner" style="--unit:${unit.color}">
        ...
        <div class="lesson-banner-copy"><span class="eyebrow">Unit ${unit.id} • Lesson ${index + 1}</span><h1>${escapeHTML(lesson.title)}</h1><p>${escapeHTML(lesson.focus)}</p><div class="lesson-meta"><span>📚 ${stations.length} Stations</span><span>🎯 30 Questions</span><span>⭐ ${stationProgress(lesson)}% Progress</span></div></div>
  ...
  function renderPhonics(lesson) {
    ...
    return `<div class="phonics-card"><div class="phonics-sound"><span>Sound Focus</span><h3>${escapeHTML(p.title)}</h3><b>${escapeHTML(p.sound)}</b><p>${escapeHTML(p.note)}</p></div><div class="sound-words">${p.words.map((word) => `<button data-speak="${escapeHTML(word)}"><span>${escapeHTML(word)}</span> 🔊</button>`).join('')}</div></div><div class="station-footer">${stationCompleteButton(lesson, 'phonics')}</div>`;
  }
  ...
  function prepareChallenge(kind, id) {
    ...
    if (kind === 'lesson') {
      ...
      subtitle = 'Lesson Challenge • 30 Questions';
      ...
      subtitle = 'Unit Challenge • 50 Questions';
      image = unit.cover;
    }
  ...
  function renderReview(page, reviewId) {
    ...
    if (!activeChallenge || activeChallenge.kind !== 'review' || activeChallenge.id !== reviewId) {
      activeChallenge = { kind: 'review', id: reviewId, title: review.title, subtitle: `${review.subtitle} • 60 Questions`, image: reviewId === 'review1' ? C.units[2].cover : C.units[5].cover, questions: QE.buildReviewQuestions(review, C) };
  ...
  window.addEventListener('p3:student', () => render());
