(() => {
  ...
  function layout(content,active='#home'){
    ...
    app.innerHTML=`
      <div class="app-frame">
        <aside class="sidebar">
          <a class="brand" href="#home"><span>ENGLISH</span><strong>PRIMARY 1</strong><em>GAME WORLD</em></a>
  ...
  function renderHome(){
    ...
    const content=`
      <section class="game-hero">
        <img src="assets/images/hero.jpg" alt="English Primary 1 Game World">
        <div class="hero-overlay"><span class="eyebrow">WELCOME TO</span><h1>English Primary 1<br><em>Game World!</em></h1><p>Explore six colorful worlds, master 48 games, and become an English star!</p><div class="hero-buttons"><a class="btn primary" href="#units">Start Adventure ▶</a><a class="btn secondary" href="#tournaments">View Tournaments 🏆</a></div></div>
        <div class="hero-dashboard"><span><b>${overallPercent()}%</b> Overall</span><span><b>${totalCompleted()}</b>/${allGames.length} Games</span><span><b>${state.level}</b> Level</span></div>
      </section>
  ...
  function renderAchievements(){const badges=[['First Kick','Complete your first game','⚽',totalCompleted()>=1],['Racing Rookie','Complete a racing game','🏎️',allGames.some(g=>g.engine==='racing'&&state.completedGames[g.id])],['Grammar Master','Score 90% in any game','📘',Object.values(state.bestScores).some(s=>s>=90)],['World Explorer','Open every unit','🗺️',curriculum.units.every(u=>unitPercent(u)>0)],['Game Champion',`Complete all ${allGames.length} games`,'🏆',totalCompleted()===allGames.length],...state.badges.map(b=>[b,'Special achievement','🌟',true])];layout(`<section class="page-banner achievement-banner"><span class="eyebrow">TROPHY ROOM</span><h1>Your Achievements</h1><p>Every badge tells the story of your learning adventure.</p></section><section class="achievement-grid">${badges.map(([t,d,i,on])=>`<article class="achievement-card ${on?'earned':'locked'}"><span>${i}</span><h3>${escapeHtml(t)}</h3><p>${escapeHtml(d)}</p><b>${on?'Earned ✓':'Locked 🔒'}</b></article>`).join('')}</section>`,'#achievements')}

  function renderProgress(){const rows=curriculum.units.map(u=>`<article class="progress-unit glass-panel"><img src="${u.cover}" alt=""><div><span>UNIT ${u.id}</span><h3>${escapeHtml(u.title)}</h3><div class="wide-progress"><i style="width:${unitPercent(u)}%"></i></div><small>${unitPercent(u)}% complete · ${u.lessons.reduce((s,l)=>s+completedCount(l.id),0)}/${u.lessons.length*2} games</small></div><a href="#unit/${u.id}">Open →</a></article>`).join('');layout(`<section class="progress-summary"><div><span class="eyebrow">MY JOURNEY</span><h1>${overallPercent()}% Complete</h1><p>You have completed ${totalCompleted()} of ${allGames.length} games.</p></div><div class="big-ring" style="--p:${overallPercent()}"><b>${overallPercent()}%</b></div><div class="summary-chips"><span>⭐ ${state.points} points</span><span>🪙 ${state.coins} coins</span><span>🛡️ ${state.badges.length} badges</span><span>🔥 ${state.streak} day streak</span></div></section><section class="progress-list">${rows}</section>`,'#progress')}
