(() => {
  ...
  const normalizeQuestion = (q) => {
    ...
    if (q.type === 'fill') {
      const answer = String(q.answer || (q.accepted || [])[0] || '').trim();
      const alphabet = ['a','b','c','d','e','f','g','h','i','j','k','l','m','n','o','p','r','s','t','u'];
      const pool = answer.length === 1 ? alphabet : ['book','chair','hello','green','family','animal'];
      const distractors = shuffle(pool.filter((x) => x.toLowerCase() !== answer.toLowerCase())).slice(0, 2);
      return { ...q, options: shuffle([answer, ...distractors]), answer };
    }
    if (q.type === 'reorder') {
    ...
    if (q.type === 'dragMatch' || q.type === 'matching') {
      const pairs = q.pairs || [];
      const first = pairs[0] || { word: 'Word', meaning: 'Picture' };
      const left = first.left || first.sentence || first.word || 'Word';
      const right = first.right || first.answer || first.meaning || 'Picture';
      return { ...q, prompt: `Match: ${left}`, options: shuffle(pairs.map((p) => p.right || p.answer || p.meaning)), answer: right };
    }
  ...
  function questionCard(q, onAnswer, title = 'Choose the correct answer') {
    ...
    wrap.innerHTML = `
      ...
      ${(q.audioPath || q.audioText) ? '<button class="listen-question" type="button">🔊 Listen again</button>' : ''}
      <div class="question-options"></div>
      <p class="question-feedback" aria-live="polite"></p>`;
    const listenButton = $('.listen-question', wrap);
    if (listenButton) listenButton.addEventListener('click', () => speak(q.audioText || q.answer, q.audioPath || ''));
  ...
  function phonics(ctx) {
    const ui = shell(ctx, ctx.game.title, 'Listen, then tap the words with the target sound.');
    const letterGroups = ctx.lesson.phonics?.letters || [];
    const targetItems = letterGroups.flatMap((letter) => (letter.words || []).map((item) => ({ ...item, letter })));
    const fallbackItems = (ctx.lesson.vocab || []).map((v) => ({ word: v.word, audio: '', icon: v.icon, letter: null }));
    const items = targetItems.length ? targetItems : fallbackItems;
    const targetWords = items.map((item) => item.word);
    const distractors = ['book','chair','sun','fish','bag','tree','cat','dog'].filter((word) => !targetWords.includes(word));
    let caught = 0, misses = 0, time = 42, combo = 0, timer;
    const letterText = letterGroups.map((l) => `${l.upper}${l.lower} ${l.sound}`).join(' · ') || 'Target Sound';
    ui.stage.innerHTML = `<div class="phonics-scene"><div class="phonics-banner glass-panel"><span>Listen to the sound</span><h2>${escapeHtml(letterText)}</h2><button class="hear-target" type="button">🔊 Hear the sound</button><p>Catch 8 correct words. Tap any word to hear it.</p></div><div class="phonics-arena"></div><div class="phonics-meter">Caught <b data-caught>0</b>/8 <span>⏱ <b data-time>42</b>s</span></div></div>`;
    const arena = $('.phonics-arena', ui.stage);
    $('.hear-target', ui.stage).onclick = () => {
      const first = letterGroups[0];
      if (first) speak(first.sound.replace(/[\/]/g, ''), first.soundAudio || '');
    };
    const spawn = () => {
      if (caught >= 8 || time <= 0 || misses >= 5) return;
      const isTarget = Math.random() < 0.7;
      const item = isTarget ? items[Math.floor(Math.random() * items.length)] : { word: distractors[Math.floor(Math.random() * distractors.length)], audio: '', icon: '✨' };
      const chip = document.createElement('button');
      chip.className = 'falling-word';
      chip.innerHTML = `<span>${item.icon || '🔤'}</span>${escapeHtml(item.word)}`;
      chip.dataset.target = isTarget ? '1' : '0';
      chip.style.left = `${5 + Math.random() * 82}%`;
      chip.style.animationDuration = `${5 + Math.random() * 2.5}s`;
      chip.onclick = () => {
        speak(item.word, item.audio || '');
        const ok = chip.dataset.target === '1';
        if (ok) { caught++; combo++; toast(ui, 'Great listening!', true); }
        else { misses++; combo = 0; toast(ui, 'Different sound. Listen and try again.', false); }
        chip.remove(); update();
      };
      chip.addEventListener('animationend', () => { if (chip.dataset.target === '1') { misses++; combo = 0; update(); } chip.remove(); });
      arena.appendChild(chip);
    };
    const update = () => {
      $('[data-caught]', ui.stage).textContent = caught;
      $('[data-time]', ui.stage).textContent = time;
      ui.score.textContent = Math.round(caught / 8 * 100);
      ui.combo.textContent = combo;
      ui.lives.textContent = Math.max(0, 5 - misses);
      if (caught >= 8 || time <= 0 || misses >= 5) {
        clearInterval(timer);
        setTimeout(() => finish(ctx, caught / 8 * 100, `You caught ${caught} target-sound words.`), 500);
      }
    };
    timer = setInterval(() => { time--; if (Math.random() < 0.8) spawn(); update(); }, 1000);
