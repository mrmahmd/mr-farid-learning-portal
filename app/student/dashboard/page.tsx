  ...
  return (
    <InnerPageShell className="student-dashboard-page">
      <section className="student-dashboard-card">
        ...
        <section className="dashboard-section"><div className="dashboard-section-heading"><div><div className="panel-kicker">My Favourite Curricula</div><h2>Choose up to two</h2></div><Link href="/student/curricula">Browse all curricula →</Link></div><div className="favourite-grid">{favorites.map((course) => { const row = rows.find((item) => item.app_id.startsWith(course.slug)); const percent = progressPercent(course, row); return <article className="favourite-course-card" key={course.slug}><div className="favourite-course-icon">{course.type === "english" ? "A" : "+"}</div><div><h3>{course.title}</h3><p>{percent}% complete</p><div className="dashboard-progress"><span style={{ width: `${percent}%` }} /></div></div><Link href={`/courses/${course.slug}`}>Open</Link></article>; })}{favorites.length < 2 && <button className="favourite-empty-card" type="button" onClick={() => saveFavorites([...favoriteSlugs, available.find((item) => !favoriteSlugs.includes(item.slug))?.slug ?? "english-primary-1"])}>+ Add favourite curriculum</button>}</div></section>
